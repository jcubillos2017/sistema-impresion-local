...
const Layout = () => {
  ...
  const menuItems = [
    ...
    { path: '/usuarios', label: 'Maestro Usuarios', icon: Users, role: 'admin' }, // Solo admin
    ...
    { path: '/org-prints', label: 'Impresion por Departamento', icon: Building2 },
    // NUEVO ENLACE MODULO 8:
    { path: '/green-state', label: 'Estado Verde', icon: Leaf },
    // NUEVO ENLACE MODULO 9:
    { path: '/pre-invoice', label: 'Pre-Factura', icon: FileSpreadsheet, role: 'admin' }, // Solo admin
    { path: '/invoice-history', label: 'Historial Pre-Facturas', icon: FileSpreadsheet, active: location.pathname === 'invoice-history' }
  ...
  return (
    <div className="flex h-screen bg-slate-50">
      ...
      <aside className={`${isSidebarOpen ? 'w-64' : 'w-20'} bg-slate-900 text-white transition-all duration-300 flex flex-col shadow-xl`}>
        
        {/* Logo / Título */}
        <div className="h-16 flex items-center justify-center border-b border-slate-700">
          {isSidebarOpen ? (
            <h1 className="text-xl font-bold text-accent tracking-wider">GESTIÓN TI</h1>
          ) : (
